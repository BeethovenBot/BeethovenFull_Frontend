...
function App() {
  ...
  async function iniciarCaptura() {
    try {
      ...
      intervalRef.current = setInterval(capturarPantallaYEnviar, 5000);
    } catch (err) {
...
async function detenerCaptura() {
  ...
  return (
    ...
    <h2 className="text-xl font-semibold mt-6">📜 Historial de Snapshots</h2>
    <div className="max-w-xl w-full">
      {historial.map((snap, index) => (
        <div key={index} className="bg-gray-800 p-4 my-2 rounded-lg">
          <p><strong>🕒</strong> {snap.timestamp}</p>
          <p><strong>🃏 Cartas jugador:</strong> {snap.cartas_jugador.join(', ')}</p>
          <p><strong>🃎 Cartas mesa:</strong> {snap.cartas_mesa.join(', ')}</p>
          <p><strong>🎯 Botón:</strong> {snap.boton_posicion}</p>
          <p><strong>🪙 Apuestas:</strong> {snap.apuestas.join(', ')}</p>
          <p><strong>💼 Stacks:</strong> {snap.stacks.join(', ')}</p>
          <p><strong>🪙 Pote:</strong> {snap.pot}</p>
        </div>
      ))}
    </div>
    </div>
